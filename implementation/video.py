...
def asvideo(video_source, factor=1, maxcols=None, lock=None):
    ...
    sample_filename = sample_filename[1]# CLOSE ONE OF THESE FILES !!
    ...
    fpipe = subp.Popen(ffmpeg_options,stdout=subp.PIPE,stderr=subp.PIPE,close_fds=True)
    fpipe.communicate()

    sample_filename = sample_filename + '1.png'
    sample_img = pylab.imread(sample_filename) # CLOSE ONE OF THESE FILES !!
